...
# define summary ops for output
values = tf.constant ([['(' + str (b) + ', ' + str (a) + ')' for b in range (1, 11)] for a in range (1, 51)])
y_conv_summary = tf.scalar_summary (values, y_conv, name='Output_Slice')
...
# set up tensor board
merged_summary_ops = tf.merge_all_summaries ()
summary_writer = tf.train.SummaryWriter ('./logs/', sess.graph_def)
summary_writer.add_graph (accuracy.graph.as_graph_def ())

# define a training saver
...
for a in range (2000):
	...
	if a % 100 == 0:
		...
		summary_str = sess.run (merged_summary_ops, feed_dict={
				x: batch [0],
				y_: batch [1],
				keep_prob: 1.0,
			})
		summary_writer.add_summary (summary_str, a)
		...
		# save model
		saver.save (sess, './saves/s', global_step = a)
